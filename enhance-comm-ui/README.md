...
- **Party buff modes** (`partyBuffMode`, default `auto`): cycle via **Buffs ·** chip on the Party panel header — `auto` (all ≤8 chips, else observed-only) · `all` · `observed` · `compact` (max 2+N) · `shared` (one unique strip per party) · `off`
